...
//Giao diện Tìm Phim theo QUốc GIa
    ...
    const fetchData = async () => {
      try {
        ...
        const [moviesRes, countriesRes] = await Promise.all([
          axios.get(`${config.API_URL}/movies/country/${slug}`),
          axios.get(`${config.API_URL}/genres`),
        ]);

        setMovies(moviesRes.data.movies);
        setGenres(countriesRes.data.genres || []);
    ...
    return movies.filter((movie) => {
      ...
      const genreMatch = selectedGenre
        ? movie.category &&
          ...
          movie.category.some((c) => c.slug === selectedGenre)
        : true;
